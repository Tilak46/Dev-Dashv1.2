...
import { Play, Globe, Settings2, Trash2, Layers, Monitor } from "lucide-react";
...
export function AppWorkspaceCard({ workspace, createProjectsLookup, onLaunch, onDelete }: AppWorkspaceCardProps & { createProjectsLookup?: any }) {
    ...
    <div className="group relative overflow-hidden rounded-xl border border-white/10 bg-black/40 backdrop-blur-md hover:border-primary/50 transition-all duration-500 hover:shadow-[0_0_30px_-5px_rgba(124,58,237,0.1)] flex flex-col">
        ...
        <div className="relative p-5 flex justify-between items-start">
            <div className="space-y-1">
                <h3 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
                    {workspace.icon ? <span className="text-2xl">{workspace.icon}</span> : <Layers className="text-primary" />}
                    {workspace.name}
                </h3>
        ...
        <div className="relative px-5 py-2 space-y-4 flex-grow">
            ...
            <div className="flex gap-2 items-center text-sm font-medium">
                {/* VS Code Projects */}
                {projectCount > 0 && (
                     <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-white/5 border border-white/5" title="Projects">
                ...
                {/* Browsers */}
                {browserCount > 0 && (
                     <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-white/5 border border-white/5" title="Browser Tabs">
                ...
                {/* Apps */}
                {appCount > 0 && (
                     <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-white/5 border border-white/5" title="Apps">
                        <Settings2 size={14} className="text-emerald-400" />
                        <span className="text-white/80">{appCount}</span>
                     </div>
                )}
                {projectCount === 0 && browserCount === 0 && appCount === 0 && (
                     <span className="text-muted-foreground text-xs italic">Empty workspace</span>
                )}
            </div>
        </div>
